...
2. **格式核对**
   对文档进行格式质量检查，覆盖标题序号连续性、日期写法统一、金额格式、标点全半角、释义简称、表格规范等 14 个核对项，输出按严重程度分级的问题清单，每条包含位置、原文、问题与修改建议。
...
对文档执行格式检查，核对完成后在文档同目录生成核对报告（总览及分级明细）。支持按文字、数据、表格类别或指定核对项执行。具体操作命令见附录。
...
| 数据 | 金额格式 | 检查千分位与两位小数；百分比、文号、标准号等不计入 |
| 数据 | 指标数值一致 | 检查同名指标前后数值不一致 |
| 数据 | 表格计算 | 检查合计行加总、占比列合计 |
| 数据 | 跨表勾稽 | 检查跨表同名科目数值 |
...
python scripts/check_content.py --input 文档.docx --checks text,data  # 按类别执行
