...
## 格式核对模式（check_content.py，只读不改文件 · 分组框架）

触发：「核对格式」「格式自查」「投行格式核对」「检查序号/金额/日期/标点」「文件质量核查」等**只读审查类请求**。适用于任意投行 Word 文档（中性通用规则）。

```bash
python scripts/check_content.py --input <docx>                          # 全部核对项
python scripts/check_content.py --input <docx> --checks text            # 只查文字类
python scripts/check_content.py --input <docx> --checks data,table      # 数据类+表格类
python scripts/check_content.py --input <docx> --checks calc,geo        # 指定子项
```

**三大组别 × 严重程度矩阵**（HIGH=错误须改 / MEDIUM=警告大概率改 / LOW=提示人工酌情）：
...
| **数据类 data** | 金额千分位与两位小数。豁免：比例%（+%）与文号/编码（6~8 位纯数字） | MEDIUM | 千分位/小数位正则 + 百分比剥离 |
| | 同名指标数值前后一致（营业收入/净利润/总资产…跨段落） | MEDIUM | 「指标名+数值+单位」提取同名比对（口径差异请人工复核） |
| | 表格合计行求和 / 占比列合计≈100%±1% | HIGH | 表格结构化解析求和容差校验 |
| | 跨表同名科目数值比对（口径不同属正常，标注供复核） | MEDIUM | 首列同名科目行的数值序列比对 |
