...
displayName: 投行文档格式
summary: A股 IPO 文档样式应用 Skill——招股书/反馈回复格式一键套用，模板即样式源可自定义，内置校验门禁。
description: >
  A股 IPO 文档样式应用 Skill：在写 Word 或改 Word 文件时，按招股书 / 审核问询反馈回复的样式要求自动应用格式
  （报告/备忘录/尽调等正式文档同归招股书版 000-009）。两套样式体系（招股书版 000-009、反馈回复版 0011/001/000-009）
  全部来自模板实证；模板即样式源——修改 assets/templates/ 下模板即定制输出样式；三场景自动路由（整套套样式/新建/局部微调）；
  内置校验门禁（必备样式/裸段落/空段落/标题跳级/内容完整性，严禁修改原文内容）；校验脚本零依赖纯标准库。
  触发词：「按招股书样式」「招股书格式」「招股书排版」「按反馈回复样式」「问询回复格式」「反馈回复排版」「落实函格式」
  「套用 000-009 样式」「报告模板样式」「套模板样式」「把这个 Word 改成XXX格式」「按投行规范排版」「排版规范」。
agent_created: true
version: 0.3.0
---
