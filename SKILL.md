...
description: >
  ...
  严重程度分级报告——标题层级序号连续性、用词规范、日期写法统一、多余空格与重复标点、
  释义简称统一、国家/城市表述合规、金额千分位与两位小数、指标数值一致、表格合计与占比
  计算校验、跨表勾稽、表格字号体系、数字右对齐、空单元格提示、不适用符号统一。
  本 Skill 仅处理格式与核查，不修改文档原文内容。
  触发词：「按招股书样式」「招股书格式」「招股书排版」「按反馈回复样式」「问询回复格式」
  「反馈回复排版」「落实函格式」「套用 000-009 样式」「报告模板样式」「套模板样式」
  「核对格式」「格式自查」「投行格式核对」「检查序号/金额/日期/标点」「文件质量核查」
version: 0.7.1
---
