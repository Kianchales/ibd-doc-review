...
description: >
  A 股 IPO 文档格式处理 Skill。对 Word 文档应用招股书（000-009 样式体系）或审核问询反馈回复
  （0011/001 体系）的排版样式；报告、备忘录、尽调报告等正式文档同归招股书版。样式定义来自
  assets/templates/ 下的模板文件，修改模板即可调整输出样式。附带 check_styles.py 校验脚本，
  检查必备样式、空段落、标题层级与内容完整性。本 Skill 仅处理格式，不修改文档原文内容。
...
A股 IPO 文档样式应用（2026-08-25 创建，封装投行文档样式规范资产）。核心：把「招股书版（000-009，含报告/备忘录/尽调等正式文档） / 审核问询反馈回复版（0011/001/000-009）」两套文档的**样式体系知识**（模板路径、pStyle 映射、序号规则、表格规则）集中封装，并在写 Word / 改 Word 时路由到底层工具执行。**管格式，不管内容**。
...
- 格式修改清单：`--diff <原文.docx>`（对比修正稿与原文，生成格式问题清单 + 修改统计，详见 S7）
...
1. **修正稿**（docx）：格式修正后的文件；**改动段落以黄色高亮标出**（脚本兜底生成时自动加 `w:highlight`；tencent-docx/minimax 等工具路径在能力范围内以高亮或批注标出）
2. **格式问题清单**（md）：`check_styles.py --diff <修正稿.docx> <原文.docx>` 自动生成 `<修正稿名>_格式修改清单.md`——逐条列出**位置、原文、改成什么**（段落位置 + 原样式 → 新样式；空段落删除单列）
