...
| 项 | 核对内容 | 判定逻辑 |
|----|---------|---------|
| 1 | 标题层级序号 | 层次感知算法：同层须递增 +1（跳号/重号/倒退均报），降入更低层重新起 1，回升高层按自身序列续；支持一~九十九中文数字与阿拉伯混写 |
...
| 4 | 释义简称 | 提取全部「（以下简称X）」定义对：同名简称指向不同全称 → 冲突警告；定义后全称复用≥3 次 → 切换提示；引号风格不统一 → 提示 |
| 5 | 中英文标点 | **前后字符判定法**（2026-08-27 裁定）：半角标点的相邻非空字符任一侧为中文 → 应为全角；全角标点两侧均为英文/数字 → 应为半角；其余间隔场景默认中文标点不报。中文语境直排引号 " 单独提示 |
| 6 | 表格填写 | 空单元格统计、单元格首尾空格、「不适用」标记符号在同表内混用检测 |
