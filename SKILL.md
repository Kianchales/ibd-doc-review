...
触发：「核对格式」「格式自查」「投行格式核对」「检查序号/金额/日期/标点」「文件质量核查」等**只读审查类请求**。
...
| 2 | 金额数字 | ≥5 位数字应加千分位；千分位金额小数位应为两位；8 位形似日期的自动标注「疑似日期请人工确认」 |
...
| 5 | 中英文标点 | 中文语境半角标点（,;:?! . ( )）、英文字母后全角标点、括号全半角混用——输出为「疑似」级供人工确认 |
