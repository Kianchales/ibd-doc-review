...
slug: ipo-doc-formatting
displayName: 投行文档格式
summary: A 股 IPO 文档格式处理：招股书/反馈回复样式排版，模板可自定义，含格式校验。
description: >
  A 股 IPO 文档格式处理 Skill。对 Word 文档应用招股书或审核问询反馈回复的排版样式；
  报告、备忘录、尽调报告等正式文档同归招股书版。样式定义来自 assets/templates/ 下的
  模板文件，修改模板即可调整输出样式。附带 check_styles.py 校验脚本，检查必备样式、
  空段落、标题层级与内容完整性。本 Skill 仅处理格式，不修改文档原文内容。
  触发词：「按招股书样式」「招股书格式」「招股书排版」「按反馈回复样式」「问询回复格式」
...
A 股 IPO 文档格式处理 Skill，负责 Word 文档的样式应用。

支持两套样式体系：招股书版（000-009，含报告、备忘录、尽调报告等正式文档）与反馈回复版（0011/001/000-009）。样式规则以模板实证为准，集中封装模板路径、pStyle 映射、序号规则与表格规则。

本 Skill 只处理格式，不修改文档内容。
...
### S5 标题序号 + 表格规则
- 标题序号九级链 + 「序号段落判定（标题 vs 正文，双维度算法）」→ `references/rules.md` 第一节
...
## 格式核对模式（check_content.py，只读不改文件）

触发：「核对格式」「格式自查」「投行格式核对」「检查序号/金额/日期/标点」「文件质量核查」等**只读审查类请求**。适用于任意投行 Word 文档（中性通用规则）。

```bash
python scripts/check_content.py --input <docx>                    # 六大项全查，报告写入 <input>_格式核对报告.md
python scripts/check_content.py --input <docx> --checks 135       # 只查 1/3/5 项
```

| 项 | 核对内容 | 判定逻辑 |
|----|---------|---------|
| 1 | 标题层级序号 | 层次感知算法：同层须递增 +1（跳号/重号/倒退均报），降入更低层重新起 1，回升高层按自身序列续；支持一~九十九中文数字与阿拉伯混写。**自定义编号参与连续性判断**（2026-08-27 裁定）：「第X节/第X章」「问题X」级别高于「一、」|
| 2 | 金额数字 | ≥5 位数字应加千分位；千分位金额小数位应为两位。**豁免**（2026-08-27 裁定）：比例数值（数字+%）不检查；6~8 位视为文号/编码跳过 |
| 3 | 日期写法 | 统计连写式 YYYYMMDD / 点横线分隔式 / 斜杠式 / 中文「年月日」四种形式占比，报主导写法、分隔符内部一致性、少数派实例清单 |
| 4 | 释义简称 | 提取全部「（以下简称X）」定义对：同名简称指向不同全称 → 冲突警告；**简称首次定义前已出现 → 问题**；括号注释短语未定义但正文高频独立复用 → 提示级疑似未定义；引号风格不统一 → 提示 |
| 5 | 中英文标点 | **前后字符判定法**（2026-08-27 裁定）：半角标点的相邻非空字符任一侧为中文 → 应为全角；全角标点两侧均为英文/数字 → 应为半角；其余间隔场景默认中文标点不报。中文语境直排引号 " 单独提示 |
| 6 | 表格填写 | 单元格首尾空格、「不适用」标记符号在同表内混用检测；**空单元格为提示级**（2026-08-27 裁定），不计入问题数 |

交付：控制台摘要 + `<input>_格式核对报告.md`（核对总览表 + 分项问题明细：位置/原文/问题/建议）。**只读核对，不改文件**——修改建议需人工确认后另行处理。
